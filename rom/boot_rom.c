...
// Strict versioning (reject equal), size policy, zero-padding enforcement.
...
#ifndef OTP_ALLOWED_MASK
#define OTP_ALLOWED_MASK 0x1u
#endif
...
static int load_file(const char* path, uint8_t** out, size_t* out_len) {
  ...
  *out = (uint8_t*)malloc((size_t)n); if (!*out) { fclose(f); return -5; }
  if (fread(*out, 1, (size_t)n, f) != (size_t)n) { fclose(f); free(*out); return -6; }
  fclose(f); *out_len = (size_t)n; return 0;
}
...
static uint32_t otp_read(void){
  ...
  if (!f) return 0;
  uint32_t v = 0;
  if (fread(&v, 1, sizeof(v), f) != sizeof(v)) { fclose(f); return 0; }
  fclose(f); return v;
}
static void otp_write(uint32_t v){
  FILE* f = fopen("out/otp_counter.bin","wb"); if (!f) return;
  (void)fwrite(&v, 1, sizeof(v), f); fclose(f);
}
...
static int verify_slot(const char* hdr_path, const char* fw_path) {
  ...
  for (size_t i = pad_off; i < (size_t)HDR_SIZE; i++) {
    if (hdr[i] != 0) { printf(C_RED "[-] Header padding is non-zero\n" C_RST); goto fail; }
  }

  uint8_t pk_hash[32];
  if (sha256(pk, h.pk_len, pk_hash) != 0) { printf(C_RED "[-] pk hash calc failed\n" C_RST); goto fail; }
  int key_index=-1;
    for (size_t i=0;i<(sizeof(OTP_PK_HASHES)/32);i++){
      if(memcmp(pk_hash, OTP_PK_HASHES[i],32)==0){ key_index=(int)i; break; }
    }
    if(key_index<0){ printf(C_RED "[-] PK not in OTP set\n" C_RST); goto fail; }
    if(((OTP_ALLOWED_MASK >> key_index)&1u)==0){ printf(C_RED "[-] PK present but revoked (idx=%d)\n" C_RST, key_index); goto fail; }
